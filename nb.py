import re
import math
with open('train.txt') as train, open('test.txt')as test ,open('predict.txt','w') as prd:
    ...
    for key, value in class1wordcount.items():
        class1proboftokens[key] = math.log10(value / class1sumwords)
    #print(sorted(class1wordcount.items(), key=lambda x: x[1]))
    #print(sorted(class1proboftokens.items(), key=lambda x: x[1]))

    class2sumwords = sum(class2wordcount.values())
    class2proboftokens = {}
    for key, value in class2wordcount.items():
        class2proboftokens[key] = math.log10(value / class2sumwords)
    #print(sorted(class2wordcount.items(), key=lambda x: x[1]))
    #print(sorted(class2proboftokens.items(), key=lambda x: x[1]))
    ...
    for line in testtxt:
        ...
        for word in line:
            ...
            if word not in class1proboftokens:
                probofthislineinclass1 += 0#class1proboftokens['unk']
            else:
                probofthislineinclass1 += class1proboftokens[word]

            if word not in class2proboftokens:
                probofthislineinclass2 += 0#class2proboftokens['unk']
            else:
